...
try {
    // Use LEFT JOINs to prevent missing data from breaking the query
    $stmt = $conn->prepare("SELECT c.*, 
                           ...
                           COALESCE(pu.email, '') as processed_by_email
                           FROM claims c
    ...
    if (empty($claims)) {
        ...
        if ($claim_count == 0) {
            set_flash_message('info', 'No claims found in database. <a href="../add_sample_data.php">Click here to add sample data</a> or <a href="../debug_claims.php">debug the issue</a>.');
        } else {
            set_flash_message('warning', "Found $claim_count claims in database but JOIN query returned no results. <a href='../debug_claims.php'>Click here to debug</a>.");
        }
    }
} catch (PDOException $e) {
    $claims = [];
...
if (($action === 'view' || $action === 'process') && $claim_id) {
    $stmt = $conn->prepare("SELECT c.*, 
                           ...
                           ph.premium_amount, p.coverage_amount, ph.start_date, ph.end_date,

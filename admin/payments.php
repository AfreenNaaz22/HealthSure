...
try {
    ...
    if (empty($payments)) {
        ...
        if ($payment_count == 0) {
            set_flash_message('info', 'No payments found in database. <a href="../add_sample_data.php">Click here to add sample data</a>.');
        } else {
            set_flash_message('warning', "Found $payment_count payments in database but JOIN query returned no results. <a href='../debug_claims.php'>Click here to debug</a>.");
        }
    }
} catch (PDOException $e) {
    $payments = [];
...
if ($action === 'view' && $payment_id) {
    $stmt = $conn->prepare("SELECT p.*, 
                           ...
                           ph.premium_amount, po.coverage_amount, ph.start_date, ph.end_date,

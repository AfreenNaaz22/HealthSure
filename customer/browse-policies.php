...
// Allow both customers and agents to view policies
$user_role = $_SESSION['role'] ?? null;
if ($user_role !== 'customer' && $user_role !== 'agent') {
    redirect('../auth/login.php');
}

// Get customer or agent info based on role
if ($user_role === 'customer') {
    $customer_info = get_customer_info($_SESSION['user_id'], $conn);
} else {
    // For agents, we don't need customer info for browsing policies
    $customer_info = null;
}
                    ...
                    <div class="col-4 policy-card" data-type="<?php echo $policy['policy_type']; ?>">
                        <div class="card mb-4">
                            ...
                            <div class="card-footer">
                                ...
                                <?php else: ?>
                                    <button class="btn btn-outline" style="width: 100%;" disabled title="Agents can view policies but cannot apply directly">
                                        View Only (Agent)
                                    </button>
                                <?php endif; ?>
                            </div>

...
try {
    ...
    // Debug information
    if (empty($my_claims)) {
        // Check if customer has claims at all
        $stmt = $conn->prepare("SELECT COUNT(*) as count FROM claims c 
                               LEFT JOIN policy_holders ph ON c.holder_id = ph.holder_id 
                               WHERE ph.customer_id = ?");
        $stmt->execute([$customer_info['customer_id']]);
        ...
        if ($claim_count == 0) {
            // Check if customer has any policy holders
            $stmt = $conn->prepare("SELECT COUNT(*) as count FROM policy_holders WHERE customer_id = ?");
            $stmt->execute([$customer_info['customer_id']]);
            $holder_count = $stmt->fetch()['count'];
            
            if ($holder_count == 0) {
                set_flash_message('info', 'You need to have an active policy before filing claims. <a href="browse-policies.php">Browse available policies</a>.');
            } else {
                set_flash_message('info', 'You haven\'t filed any claims yet. <a href="file-claim.php">File your first claim</a>.');
            }
        } else {
            set_flash_message('warning', "Found $claim_count claims but unable to display them. <a href='../debug_claims.php'>Click here to debug</a>.");
        }
    }
} catch (PDOException $e) {
    $my_claims = [];
    set_flash_message('danger', 'Database error: ' . $e->getMessage() . ' <a href="../debug_claims.php">Click here to debug</a>.');
}
